...
//	    log.Fatal(err)
//	}
func NewXML(path string, opts ...parser.Option) *XML {
	x := &XML{
		Path: path,
		options: parser.Options{
			BufferSize: 8192,
...
func (x *XML) Load() (map[string]string, error) {
	if ext := path.Ext(x.Path); ext != ".xml" {
		return nil, parser.ErrInvalidExtension.Newf("expected .xml, got %s", ext)
	}

	file, err := os.Open(x.Path)
	if err != nil {
		if os.IsNotExist(err) {
			return nil, parser.ErrFileNotFound.Wrap(err).WithTag("path", x.Path)
		}
		return nil, parser.ErrReadFailed.Wrap(err).WithTag("path", x.Path)
	}
	defer file.Close()
...
func (x *XML) LoadReader(r io.Reader) (map[string]string, error) {
	...
	if err != nil {
		return nil, parser.ErrReadFailed.Wrap(err).WithTag("parser", "xml")
	}
