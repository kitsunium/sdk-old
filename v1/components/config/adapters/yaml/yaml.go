...
//	    log.Fatal(err)
//	}
func NewYAML(path string, opts ...parser.Option) *YAML {
	y := &YAML{
		Path: path,
		options: parser.Options{
			BufferSize: 8192,
...
func (y *YAML) Load() (map[string]string, error) {
	ext := path.Ext(y.Path)
	if ext != ".yaml" && ext != ".yml" {
		return nil, parser.ErrInvalidExtension.Newf("expected .yaml or .yml, got %s", ext)
	}

	file, err := os.Open(y.Path)
	if err != nil {
		if os.IsNotExist(err) {
			return nil, parser.ErrFileNotFound.Wrap(err).WithTag("path", y.Path)
		}
		return nil, parser.ErrReadFailed.Wrap(err).WithTag("path", y.Path)
	}
	defer file.Close()
...
func (y *YAML) LoadReader(r io.Reader) (map[string]string, error) {
	...
	if err != nil {
		return nil, parser.ErrReadFailed.Wrap(err).WithTag("parser", "yaml")
	}
