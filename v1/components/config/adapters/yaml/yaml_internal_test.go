...
func TestYAML_NewYAML(t *testing.T) {
	...
	// Test with options
	y2 := NewYAML("test.yml", parser.WithBufferSize(8192), parser.WithPool(false))
...
`
	...
	yaml := NewYAML("", parser.WithPool(true))
...
`
	...
	yaml := NewYAML("", parser.WithPool(false))
	reader := strings.NewReader(content)
	result, err := yaml.LoadReader(reader)
	if err != nil {
		t.Fatalf("LoadReader() error = %v", err)
...
func TestYAML_LoadReader_InvalidYAML(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			yaml := NewYAML("", parser.WithPool(tc.pool))
