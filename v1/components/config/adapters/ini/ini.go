...
import (
	...
	"github.com/kitsunium/sdk/v1/internal/core/normalize"
)
...
//	    log.Fatal(err)
//	}
func NewINI(path string, opts ...parser.Option) *INI {
	i := &INI{
		Path: path,
		options: parser.Options{
			BufferSize: 8192,
...
func (i *INI) Load() (map[string]string, error) {
	if ext := path.Ext(i.Path); ext != ".ini" && ext != ".cfg" && ext != ".conf" {
		return nil, parser.ErrInvalidExtension.Newf("expected .ini, .cfg or .conf, got %s", ext)
	}

	data, err := os.ReadFile(i.Path)
	if err != nil {
		if os.IsNotExist(err) {
			return nil, parser.ErrFileNotFound.Wrap(err).WithTag("path", i.Path)
		}
		return nil, parser.ErrReadFailed.Wrap(err).WithTag("path", i.Path)
	}
...
func (i *INI) LoadReader(r io.Reader) (map[string]string, error) {
	...
	if err != nil {
		return nil, parser.ErrReadFailed.Wrap(err).WithTag("parser", "ini")
	}
	return i.LoadBytes(data)
}
