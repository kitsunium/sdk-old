...
func TestTOML_NewTOML(t *testing.T) {
	...
	// Test with options
	t2 := NewTOML("test.toml", parser.WithBufferSize(8192), parser.WithPool(false))
...
`
	...
	toml := NewTOML("", parser.WithPool(true))
...
`
	...
	toml := NewTOML("", parser.WithPool(false))
	reader := strings.NewReader(content)
	result, err := toml.LoadReader(reader)
	if err != nil {
		t.Fatalf("LoadReader() error = %v", err)
...
func TestTOML_LoadReader_InvalidTOML(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			toml := NewTOML("", parser.WithPool(tc.pool))
