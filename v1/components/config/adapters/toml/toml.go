...
//	    log.Fatal(err)
//	}
func NewTOML(path string, opts ...parser.Option) *TOML {
	t := &TOML{
		Path: path,
		options: parser.Options{
			BufferSize: 8192,
...
func (t *TOML) Load() (map[string]string, error) {
	if ext := path.Ext(t.Path); ext != ".toml" {
		return nil, parser.ErrInvalidExtension.Newf("expected .toml, got %s", ext)
	}

	file, err := os.Open(t.Path)
	if err != nil {
		if os.IsNotExist(err) {
			return nil, parser.ErrFileNotFound.Wrap(err).WithTag("path", t.Path)
		}
		return nil, parser.ErrReadFailed.Wrap(err).WithTag("path", t.Path)
	}
	defer file.Close()
...
func (t *TOML) LoadReader(r io.Reader) (map[string]string, error) {
	...
	if err != nil {
		return nil, parser.ErrReadFailed.Wrap(err).WithTag("parser", "toml")
	}
