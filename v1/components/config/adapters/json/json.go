...
func NewJSON(path string, opts ...parser.Option) *JSON {
	j := &JSON{
		Path: path,
		options: parser.Options{
			BufferSize: 8192,
...
func (j *JSON) Load() (map[string]string, error) {
	if ext := path.Ext(j.Path); ext != ".json" {
		return nil, parser.ErrInvalidExtension.Newf("expected .json, got %s", ext)
	}

	data, err := os.ReadFile(j.Path)
	if err != nil {
		if os.IsNotExist(err) {
			return nil, parser.ErrFileNotFound.Wrap(err).WithTag("path", j.Path)
		}
		return nil, parser.ErrReadFailed.Wrap(err).WithTag("path", j.Path)
	}
...
func (j *JSON) LoadReader(r io.Reader) (map[string]string, error) {
	...
	if err != nil {
		return nil, parser.ErrReadFailed.Wrap(err).WithTag("parser", "json")
	}
