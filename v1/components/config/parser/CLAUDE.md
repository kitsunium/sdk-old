...
| File | Role |
|---|---|
| `parser.go` | `Parser`, `FileParser` interfaces; `ParserOption`, `WithBufferSize(int)`, `WithPool(bool)`; private `baseParser` |
...
3. **Buffering**: the `WithPool(true)` option routes reads through `internal/core/pool` — keep parity across all file parsers.
...
```bash
bazel test //v1/components/config/parser/...
cd v1 && go test -race ./components/config/parser
```
