...
// ptr returns a pointer to v for test option construction. Replaces the removed
// pkg/lib/pointer helper now that Go generics make &v idiomatic.
...
func TestFile(t *testing.T) {
	...
	t.Run("NewFile", func(t *testing.T) {
		t.Run("nominal", func(t *testing.T) {
			f, err := files.NewFile(files.Option{
				Path: tmpFile.Name(),
			})
			assert.NoError(t, err)
			assert.NotNil(t, f)
		...
		t.Run("is Symbolic link", func(t *testing.T) {
			...
			_, err := files.NewFile(files.Option{
				Path:             origin,
				CreateIfNotExist: true,
			})
			assert.NoError(t, err)
