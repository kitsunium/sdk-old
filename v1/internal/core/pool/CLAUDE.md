...
| File | Role |
|---|---|
| `pool.go` | `Buffer`, `Pool`, `Sharded`, `Option`; constructors `NewSafeBuffer`, `NewUnsafeBuffer`, `NewSafeShardedBuffer`, `GetGlobalPool`; sizing & sentinel-error constants |
...
- Dead constants from `pool.go`: `likelyTrue/False`, `prefetchRead/Write`, `alignment16/32`, `ptrSize/wordSize`, `optimalIOSize`, `stateFlagLocked/ReadOnly`, shadowing `min/max` helpers.
...
```bash
bazel test //internal/core/pool/...
go test -race -bench=. -benchmem ./internal/core/pool
```
