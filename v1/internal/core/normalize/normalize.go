...
var (
	...
	separatorToDot [tableSize]byte
)

func init() {
	for i := startIndex; i < tableSize; i++ {
		toLowerTable[i] = byte(i)
		separatorToDot[i] = byte(i)
	}
	for i := asciiUpperCaseStart; i <= asciiUpperCaseEnd; i++ {
		toLowerTable[i] = byte(i + asciiCaseDiff)
	}
	//: both '_' (env style) and '-' (CLI args style) normalize to '.' so
	//: every source format agrees on the canonical dot-path form
	separatorToDot['_'] = '.'
	separatorToDot['-'] = '.'
}

// Key normalizes a configuration key by converting to lowercase and
// replacing separators with dots. This function uses pre-computed
// lookup tables for optimal performance and avoids allocations when
// possible.
//
// The normalization process:
//   - Converts uppercase letters to lowercase
//   - Replaces underscores ('_') and dashes ('-') with dots
...
//	Key("database-host") // returns "database.host"
