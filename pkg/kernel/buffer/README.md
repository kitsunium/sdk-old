...
The buffer package provides two main components optimized for performance:
...
```go
import "github.com/kitsunium/sdk/pkg/kernel/buffer"

// Create a new buffer with 1024 bytes capacity
buf := buffer.NewBuffer(1024)
...
// Use the global pool
buf := buffer.Get(1024)
defer buffer.Put(buf) // Return to pool when done
...
func processData(input []byte) ([]byte, error) {
    ...
    buf := buffer.GetBuffer(len(input) * 2)
    defer buffer.PutBuffer(buf)
...
```bash
# Run tests
go test ./pkg/kernel/buffer/...

# Check coverage (96.4%)
go test -cover ./pkg/kernel/buffer/...

# Generate coverage report
go test -coverprofile=coverage.out ./pkg/kernel/buffer/...
