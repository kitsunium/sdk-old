...
func TestAutoGeneratedMessage(t *testing.T) {
	ClearRegistry()

	// Test auto-generated message for code without message
	err1 := Define(KConfig{Code: 200})
	if err1.Message() != "error 200" {
		t.Errorf("Message should be auto-generated, got: %s", err1.Message())
	}

	// Test another code
	err2 := Define(KConfig{Code: 201})
	if err2.Message() != "error 201" {
		t.Errorf("Message should be auto-generated, got: %s", err2.Message())
	}

	// Test non-standard code
	err3 := Define(KConfig{Code: 999})
	if err3.Message() != "error 999" {
		t.Errorf("Message should be auto-generated, got: %s", err3.Message())
	}
}
...
func TestGetCallerPackageFullCoverage(t *testing.T) {
	...
	// Test 2: Cache hit - call again with same PC
	pkg2 := getCallerPackage()
	if pkg1 != pkg2 {
		t.Error("Cached package should be the same")
	}
	...
	if pkgNoSlash == "" {
		t.Error("Package should not be empty for no-slash function")
	}
	...
	go func() {
		done <- getCallerPackage()
	}()

	goroutinePkg := <-done
	if goroutinePkg == "" {
		t.Error("Package from goroutine should not be empty")
	}

	// Test 5: Test with various function name patterns
...
func TestGetCallerPackageEdgeCasesComplete(t *testing.T) {
	...
	if pkg1 != pkg2 {
		t.Error("Should return same package from cache")
	}
