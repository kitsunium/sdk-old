...
// Package kcache provides ultra-optimized, unsafe-based cache implementation
// for kernel-level operations. This file contains goroutine safety checking
// for development builds to detect concurrent access to unsafe caches.
package kcache
...
// Uses atomic operations to detect when an unsafe cache is accessed
...
func (g *goroutineChecker) checkGoroutineSafety() {
	...
	if currentOwner == 0 {
		...
		if currentOwner != uint64(currentGID) {
			panic("kcache: UNSAFE cache accessed from multiple goroutines! " +
				"Use NewSafeCache() or NewSafeShardedCache() for concurrent access. " +
