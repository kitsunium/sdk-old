...
Package files provides high-performance file system operations and utilities
for the Kitsunium kernel.

## Overview

The files package offers low-level file system operations using direct Unix system
...
```bash
go get github.com/kitsunium/sdk/pkg/kernel/files
```

## Usage

```go
import "github.com/kitsunium/sdk/pkg/kernel/files"
```
...
func directoryExample() {
    ...
    dir, err := files.NewDirectory(option)
    if err != nil {
        log.Fatal(err)
    }
...
```go
func statsExample() {
    stats := files.NewStats("/tmp/example.txt")
...
func archiveExample() {
    // Create archive
    archive, err := files.NewArchive(files.Option{
...
```bash
# Run tests
go test ./pkg/kernel/files/...

# Run with race detection
go test -race ./pkg/kernel/files/...

# Run benchmarks
go test -bench=. ./pkg/kernel/files/...

# Test with coverage
go test -cover ./pkg/kernel/files/...
```
